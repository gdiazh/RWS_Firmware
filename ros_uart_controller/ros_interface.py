...
class DRV10987Interface:
    ...
    def start(self):
        ...
        self.running = True
        #publishers
        self.data1_pub = rospy.Publisher('/current_ma_1', Float32, queue_size=70)
        self.data2_pub = rospy.Publisher('/speed_rpm_1', UInt16, queue_size=70)
        self.data3_pub = rospy.Publisher('/speed_cmd_1', UInt16, queue_size=70)
        self.data4_pub = rospy.Publisher('/current_ma_2', Float32, queue_size=70)
        self.data5_pub = rospy.Publisher('/speed_rpm_2', UInt16, queue_size=70)
        self.data6_pub = rospy.Publisher('/speed_cmd_2', UInt16, queue_size=70)
        self.data7_pub = rospy.Publisher('/current_ma_3', Float32, queue_size=70)
        self.data8_pub = rospy.Publisher('/speed_rpm_3', UInt16, queue_size=70)
        self.data9_pub = rospy.Publisher('/speed_cmd_3', UInt16, queue_size=70)
    ...
    def update_state(self):
        ...
        while self.running and not rospy.is_shutdown():
            if(self.serial_trx.read()):
                ...
                speed_rpm_1 = data[1]
                speed_cmd_1 = data[2]
                current_ma_2 = 1.46484375*(data[3]-1023)
                speed_rpm_2 = data[4]
                speed_cmd_2 = data[5]
                current_ma_3 = 1.46484375*(data[6]-1023)
                speed_rpm_3 = data[7]
                speed_cmd_3 = data[8]
                # ignore outliers
                ...
                self.data4_pub.publish(current_ma_2)
                self.data5_pub.publish(speed_rpm_2)
                self.data6_pub.publish(speed_cmd_2)
                self.data7_pub.publish(current_ma_3)
                self.data8_pub.publish(speed_rpm_3)
                self.data9_pub.publish(speed_cmd_3)
                # get time
