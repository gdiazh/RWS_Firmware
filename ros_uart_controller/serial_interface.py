...
SET_SPEED_CODE_MOTOR2 = 26
SET_SPEED_CODE_MOTOR3 = 29
...
class serialInterface(object):
        ...
        while (k < 2*self.size):
            ...
            if (i==self.size):
                ...
                if (chksm == self.packet[self.size-1] and chksm !=0 and self.packet[self.size-2] == 253 and self.packet[self.size-3] == 254 and self.packet[self.size-4] == 255):
                    ...
                    p = struct.pack('B'*self.size, self.packet[0],self.packet[1],self.packet[2],self.packet[3],self.packet[4],self.packet[5],self.packet[6],self.packet[7],self.packet[8],self.packet[9], self.packet[10],self.packet[11],self.packet[12],self.packet[13],self.packet[14],self.packet[15],self.packet[16],self.packet[17],self.packet[18],self.packet[19],self.packet[20],self.packet[21])
                    self.packet = struct.unpack("HHHHHHHHHBBBB", p)
                    self.struct = struct.unpack("HHHHHHHHHBBBB", p)
    ...
    def write(self, cmd_code, speed):
        if cmd_code == 1:
            ...
            cmd = SET_SPEED_CODE_MOTOR2
        elif cmd_code == 3:
            cmd = SET_SPEED_CODE_MOTOR3
        else:
